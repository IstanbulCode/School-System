...
module.exports = {
         ...
         if (tutor) {
            ...
            if (institution) {
               ...
               db.get().collection(collections.TUTOR_COLLECTION).insertOne(tutorData).then((data) => {
                  //tutor = data.ops[0]
                  signupMsg = "Your status is now pending. School will verify your details then you can login."
                  resolve(signupMsg)
               })
            }
            else {
               signupErr = "School does not exists. Please check and try again"
               reject(signupErr)
            }
         }
      })
            ...
            bcrypt.compare(tutorData.password, tutor.password).then((status) => {
               if (status) {
                  if (tutor.status === 'Signup verified') {
                     ...
                     loginErr = "Your status is now pending. School will verify your details then you can logged in."
                     reject(loginErr)
                  }
                  else if (tutor.status == 'Signup denied') {
                     loginErr = "Your signup request was denied by School. Please contact your School."
                     reject(loginErr)
                  }
                  else {
                     loginErr = "Your account was blocked. Please contact your School"
                     reject(loginErr)
                  }
               }
               else {
   ...
   getAllAnnouncements: function () {
      return new Promise(async (resolve, reject) => {
         let announcements = await db.get().collection(collections.ANNOUNCEMENT_COLLECTION).find({ $or: [{ visiblity: 'Everyone' }, { visiblity: 'Teacher' }] }).sort({ _id: -1 }).toArray()
